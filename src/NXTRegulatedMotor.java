...
public class NXTRegulatedMotor implements RegulatedMotor
{
    ...
    /**
     * Returns the current power that the motor regulator is commanding
     * @return the current motor power calculated by the regulator.
     */
    public int getPower()
    {
        return (reg.power);
    ...
    /**
     * Returns the current stall count that the motor regulator has calculated
     * @return the current motor stall count calculated by the regulator.
     */
    ...
    protected class Regulator
    {
        ...
        synchronized private void endMove(boolean stalled)
        {
            ...
            if (stalled)
            {
            	...
            	// TF TEST this makes a thread lock?
                reset();
                curVelocity = 0;
                stallCnt = 0;
        ...
        synchronized void regulateMotor(long delta)
        {
            ...
            if (moving)
            {
                ...
                {
                    baseTime += delta;
                    // TF TEST make a stall act like the move completed.  This makes it not lock up.
