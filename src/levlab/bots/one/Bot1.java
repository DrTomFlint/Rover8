 ...
 * Copied from Bot9 code for Bot1, 12 Dec 2019
 * */
...
public class Bot1
{
	...
	// This is the "constructor" for the class TomsBot1
	public Bot1()
	{	   
		local = Bot1shared.getInstance();
	...
	{      
		...
		Bot1monitor monitor = new Bot1monitor();
		monitor.start();

		// setup thread to update LCD
		Bot1lcd lcd = new Bot1lcd();
		lcd.start();

		// setup thread to watch the Bluetooth Comms
		Bot1comms comms = new Bot1comms();
		comms.start();

		// setup thread for reporting status to the servlet
		Bot1reporter reporter = new Bot1reporter();
		reporter.start();
	...
	private void clawSetZero(){
		...
		Claw.resetTachoCount();
		local.claw = Bot1shared.CLAW_ZERO;
		local.clawMax = -150;
		Claw.flt();
	...
	private void readData()
	{
		...
		if(local.btState != Bot1shared.BT_OK){
			...
			} catch (IOException e)
			{
				...
				command = -1;
				local.btState = Bot1shared.BT_ERROR;
			}
...
// TEST
	...
	// Classes used by Bot1
	Bot1shared local;
	Bot1lcd lcd;
	Bot1reporter reporter;
	Bot1monitor monitor;
