...
/* TF FIX:
 * This class is modified from the standard DifferentialPilot class in the robotics/navigation
 * to add threadYield() in the stop() method and remove waitComplete().  That call was blocking
 * some other thread?  So if a motor stalled, and I call pilot.stop() the motor would go fast
 * and everything would lock up requiring a ESC+ENTER on the NXT buttons to halt the program.
 * 
 * Looks like arc moves do not account for max speed or motors or acceleration limits.
 * 
 * Consider adding a method .doMove(Move) that would accept a move object and call the right
 * one of the existing methods to execute the move.
 * 
 */
 ...
 *
 **/
public class TomsPilot implements
        RegulatedMotorListener, ArcRotateMoveController
{
   ...
   */
  public TomsPilot(final double wheelDiameter, final double trackWidth,
   ...
   *            running backward.
   */
  public TomsPilot(final double leftWheelDiameter,
 ...
 /**
  ...
  public void stop()
  {
    _left.stop(true);
    _right.stop(true);
    // TF TEST can't call waitComplete as it makes some kind of thread lock
    // waitComplete();          
    // TF TEST
    Thread.yield();
    // TF TEST
    setMotorAccel(_acceleration);  // restore acceleration value
    // TF TEST after this whole bit is finished, then command motors to float not stop, this should turn off the power
    _left.flt(true);
    _right.flt(true);
  }
   ...
   * is too slow;
   */ 
  // TF BUG: _quickAcceleration is never given a value.  Manually set accel and call stop() 
  // instead of quickStop().
 ...
 * robot is traveling toward the top of the page the arc looks like this: <b>)</b>. <br>
 ...
 protected final float _leftDegPerDistance;
  ...
  // TF TEST could initialize this here to get quickStop to work?
